        ...
        <div className={`transition-all duration-700 transform ${isReady ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {GALLERY_IMAGES.map((img, index) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-emerald-950/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
                    <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      <div className="w-8 h-px bg-amber-400 mb-2"></div>
                      <p className="text-white font-serif text-lg italic">{img.alt}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="relative group max-w-4xl mx-auto">
              <div className="relative aspect-[4/5] md:aspect-[16/10] overflow-hidden rounded-[2.5rem] shadow-2xl border-4 border-white bg-gray-100">
                {GALLERY_IMAGES.map((img, index) => (
                  <div 
                    key={index}
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-10 bg-gradient-to-t from-emerald-950/90 to-transparent">
                      ...
                      <p className="text-white font-serif text-3xl italic">{img.alt}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <button 
                onClick={prevSlide}
                className="absolute left-4 top-1/2 -translate-y-1/2 w-14 h-14 bg-white/90 backdrop-blur-md text-emerald-950 rounded-full flex items-center justify-center shadow-lg hover:bg-amber-400 hover:text-white transition-all transform active:scale-90"
              >
              ...
              <button 
                onClick={nextSlide}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-14 h-14 bg-white/90 backdrop-blur-md text-emerald-950 rounded-full flex items-center justify-center shadow-lg hover:bg-amber-400 hover:text-white transition-all transform active:scale-90"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
